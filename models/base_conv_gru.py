...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur, mask=None):
        ...
        h_next = mask * h_next.clone() + (1 - mask) * h_cur.clone()
...
class Encoder_z0_ODE_ConvGRU(nn.Module):
    ...
    def run_ode_conv_gru(self, input_tensor, mask, time_steps, run_backwards=True, tracker=None):
        ...
        for idx, i in enumerate(time_points_iter):
            ...
            ode_sol = prev_input_tensor.clone() + inc
    ...
    def _check_kernel_size_consistency(kernel_size):
        if not (isinstance(kernel_size, tuple) or
            ...
            raise ValueError('`kernel_size` must be tuple or list.')
        if isinstance(kernel_size, list) and not all([isinstance(elem, tuple) for elem in kernel_size]):
